...
async function fetchUserData(url) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch data! Status Code: ${response.status}`); // Hey JS, add the value of the variable
    }
    const contentType = response.headers.get("content-type");
    console.log("Status:", response.status, "Content-Type:", contentType); // get the content-type and response.status readable in devtools
    if (!contentType.includes("application/json")) {
      console.error("Wrong content-type:", contentType);
      throw new Error("The API link is incorrect or unavailable.");
    }

    /*const data = await response.json();*/
    // mit return await schon abgedeckt
